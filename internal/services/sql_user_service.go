...
func (m *SqlUserService) GetAll(ctx context.Context) (*[]User, error) {
	...
	for rows.Next() {
		var user User
		err := rows.Scan(&user.Id, &user.Username, &user.Phone, &user.Email, &user.DateOfBirth, pq.Array(&user.Interests), pq.Array(&user.Skills), &user.Settings)
		if err != nil {
			return nil, err
		}
		res = append(res, user)
...
func (m *SqlUserService) Load(ctx context.Context, id string) (*User, error) {
	...
	for row.Next(){
		err = row.Scan(&user.Id, &user.Username, &user.Phone, &user.Email, &user.DateOfBirth, pq.Array(&user.Interests), pq.Array(&user.Skills), &user.Settings)
	}
	if err != nil {
		errMsg := err.Error()
...
func (m *SqlUserService) Insert(ctx context.Context, user *User) (int64, error) {
	query := "insert into users (id, username, email, phone, date_of_birth, interests, skills, settings) values ($1, $2, $3, $4, $5, $6, $7, $8)"
	result,err := m.DB.Exec(query, user.Id, user.Username, user.Email, user.Phone, user.DateOfBirth, pq.Array(user.Interests), pq.Array(user.Skills), user.Settings)
	if err != nil {
		return -1, nil
	}
...
func (m *SqlUserService) Update(ctx context.Context, user *User) (int64, error) {
	query := "update users set username = $2, email = $3, phone = $4, date_of_birth = $5, interests = $6, skills = $7, settings = $8 where id = $1"
	result, err := m.DB.Exec(query, user.Id, user.Username, user.Email, user.Phone, user.DateOfBirth, pq.Array(user.Interests), pq.Array(user.Skills), user.Settings)
	if err != nil {
		return -1, err
	}
